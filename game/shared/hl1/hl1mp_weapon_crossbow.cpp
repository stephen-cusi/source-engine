...
// Crossbow Bolt
//-----------------------------------------------------------------------------
class CHL1MPCrossbowBolt : public CBaseCombatCharacter
{
	DECLARE_CLASS( CHL1MPCrossbowBolt, CBaseCombatCharacter );
...
public:
	...
	static CHL1MPCrossbowBolt *BoltCreate( const Vector &vecOrigin, const QAngle &angAngles, CBasePlayer *pentOwner = NULL );
...
LINK_ENTITY_TO_CLASS( crossbow_hl1mp_bolt, CHL1MPCrossbowBolt );

BEGIN_DATADESC( CHL1MPCrossbowBolt )
	// Function Pointers
...
CHL1MPCrossbowBolt *CHL1MPCrossbowBolt::BoltCreate( const Vector &vecOrigin, const QAngle &angAngles, CBasePlayer *pentOwner )
{
	// Create a new entity with CHL1MPCrossbowBolt private data
	CHL1MPCrossbowBolt *pBolt = (CHL1MPCrossbowBolt *)CreateEntityByName( "crossbow_hl1mp_bolt" );
...
void CHL1MPCrossbowBolt::Spawn( )
{
	...
	SetTouch( &CHL1MPCrossbowBolt::BoltTouch );

	SetThink( &CHL1MPCrossbowBolt::BubbleThink );
...
void CHL1MPCrossbowBolt::Precache( )
{
	PrecacheModel( BOLT_MODEL );
	PrecacheScriptSound( "BaseGrenade.Explode" );    
}

void CHL1MPCrossbowBolt::BoltTouch( CBaseEntity *pOther )
{
...
//			NDebugOverlay::Box( GetAbsOrigin(), Vector( -16, -16, -16 ), Vector( 16, 16, 16 ), 0, 0, 255, 0, 10 );
	...
	else
	{
		...
		SetThink( &CHL1MPCrossbowBolt::SUB_Remove );
	...
	if ( g_pGameRules->IsMultiplayer() && m_bExplode )
	{
        SetThink( &CHL1MPCrossbowBolt::ExplodeThink );
        SetNextThink( gpGlobals->curtime + 0.1f );
	}
}

void CHL1MPCrossbowBolt::ExplodeThink( void )
{
...
void CHL1MPCrossbowBolt::BubbleThink( void )
{
	QAngle angNewAngles;
...
//-----------------------------------------------------------------------------
bool CHL1MPCrossbowBolt::CreateVPhysics( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
unsigned int CHL1MPCrossbowBolt::PhysicsSolidMaskForEntity() const
{
...
//-----------------------------------------------------------------------------
// CHL1MPWeaponCrossbow
//-----------------------------------------------------------------------------

#ifdef CLIENT_DLL
#define CHL1MPWeaponCrossbow C_HL1MPWeaponCrossbow
#endif

class CHL1MPWeaponCrossbow : public CWeaponHL2MPBase
{
	DECLARE_CLASS( CHL1MPWeaponCrossbow, CWeaponHL2MPBase );
public:

	DECLARE_NETWORKCLASS(); 
	DECLARE_PREDICTABLE();

	CHL1MPWeaponCrossbow( void );
...
IMPLEMENT_NETWORKCLASS_ALIASED( HL1MPWeaponCrossbow, DT_HL1MPWeaponCrossbow );

BEGIN_NETWORK_TABLE( CHL1MPWeaponCrossbow, DT_HL1MPWeaponCrossbow )
#ifdef CLIENT_DLL
...
BEGIN_PREDICTION_DATA( CHL1MPWeaponCrossbow )
#ifdef CLIENT_DLL
...
//IMPLEMENT_SERVERCLASS_ST( CHL1MPWeaponCrossbow, DT_WeaponCrossbow )
//END_SEND_TABLE()

BEGIN_DATADESC( CHL1MPWeaponCrossbow )
END_DATADESC()

#if 0
#ifndef CLIENT_DLL

acttable_t	CHL1MPWeaponCrossbow::m_acttable[] = 
{
...
IMPLEMENT_ACTTABLE(CHL1MPWeaponCrossbow);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHL1MPWeaponCrossbow::CHL1MPWeaponCrossbow( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHL1MPWeaponCrossbow::Precache( void )
{
#ifndef CLIENT_DLL
	UTIL_PrecacheOther( "crossbow_hl1mp_bolt" );
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
void CHL1MPWeaponCrossbow::PrimaryAttack( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CHL1MPWeaponCrossbow::SecondaryAttack( void )
{
	ToggleZoom();
...
void CHL1MPWeaponCrossbow::FireBolt( void )
{
	if ( m_iClip1 <= 0 )
	{
		if ( !m_bFireOnEmpty )
...
#ifndef CLIENT_DLL
	CHL1MPCrossbowBolt *pBolt = CHL1MPCrossbowBolt::BoltCreate( vecSrc, angAiming, pOwner );
...
bool CHL1MPWeaponCrossbow::Reload( void )
{
	bool fRet;
...
void CHL1MPWeaponCrossbow::WeaponIdle( void )
{
...
bool CHL1MPWeaponCrossbow::Deploy( void )
{
	if ( m_iClip1 <= 0 )
	{
...
bool CHL1MPWeaponCrossbow::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	if ( m_fInZoom )
	{
		ToggleZoom();
...
void CHL1MPWeaponCrossbow::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	if ( !pPlayer )
	{
