...
CUtlSymbolTable g_EntityButtonSoundNames;

DECLARE_BUILD_FACTORY_DEFAULT_TEXT(EntityButton, EntityButton );

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
EntityButton::EntityButton(Panel *parent, const char *panelName, const char *text, Panel *pActionSignalTarget, const char *pCmd ) : Label(parent, panelName, text)
{
	Init();
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
EntityButton::EntityButton(Panel *parent, const char *panelName, const wchar_t *wszText, Panel *pActionSignalTarget, const char *pCmd ) : Label(parent, panelName, wszText)
{
	Init();
	if ( pActionSignalTarget && pCmd )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::SetButtonActivationType(ActivationType_t activationType)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetButtonBorderEnabled( bool state )
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetSelected( bool state )
{
...
void EntityButton::SetBlink( bool state )
{
...
//-----------------------------------------------------------------------------
void EntityButton::ForceDepressed(bool state)
{
...
//-----------------------------------------------------------------------------
void EntityButton::RecalculateDepressedState( void )
{
	bool newState;
	if (!IsEnabled())
	{
...
//-----------------------------------------------------------------------------
void EntityButton::SetUseCaptureMouse( bool state )
{
...
//-----------------------------------------------------------------------------
bool EntityButton::IsUseCaptureMouseEnabled( void )
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetArmed(bool state)
{
	if ( _buttonFlags.IsFlagSet( ARMED ) != state )
	{
		...
		{
			surface()->PlaySound(g_EntityButtonSoundNames.String(m_sArmedSoundName));
		}
	}
}
...
//-----------------------------------------------------------------------------
bool EntityButton::IsArmed()
{
	return _buttonFlags.IsFlagSet( ARMED );
}


KeyValues * EntityButton::GetActionMessage()
{
	return _actionMessage->MakeCopy();
}

void EntityButton::PlayButtonReleasedSound()
{
	// check for playing a transition sound
	if ( m_sReleasedSoundName != UTL_INVAL_SYMBOL )
	{
		surface()->PlaySound( g_EntityButtonSoundNames.String( m_sReleasedSoundName ) );
	}
}
...
//-----------------------------------------------------------------------------
bool EntityButton::IsDepressed()
{
	return _buttonFlags.IsFlagSet( DEPRESSED );
}

bool EntityButton::IsBlinking( void )
{
...
// Drawing focus box?
//-----------------------------------------------------------------------------
bool EntityButton::IsDrawingFocusBox()
{
	return _buttonFlags.IsFlagSet( DRAW_FOCUS_BOX );
}

void EntityButton::DrawFocusBox( bool bEnable )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void EntityButton::NavigateFrom()
{
...
//-----------------------------------------------------------------------------
void EntityButton::PerformLayout()
{
	// reset our border
...
// Output : Color
//-----------------------------------------------------------------------------
Color EntityButton::GetButtonFgColor()
{
...
//-----------------------------------------------------------------------------
Color EntityButton::GetButtonBgColor()
{
	if (_buttonFlags.IsFlagSet( DEPRESSED ))
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::ApplySchemeSettings(IScheme *pScheme)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetDefaultColor(Color fgColor, Color bgColor)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetArmedColor(Color fgColor, Color bgColor)
{
...
// Purpose: Set armed button colors
//-----------------------------------------------------------------------------
void EntityButton::SetSelectedColor(Color fgColor, Color bgColor)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetDepressedColor(Color fgColor, Color bgColor)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetBlinkColor(Color fgColor)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetDefaultBorder(IBorder *border)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetDepressedBorder(IBorder *border)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetKeyFocusBorder(IBorder *border)
{
...
//-----------------------------------------------------------------------------
IBorder *EntityButton::GetBorder(bool depressed, bool armed, bool selected, bool keyfocus)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetAsCurrentDefaultButton(int state)
{
...
//			when the user hits ENTER or SPACE
//-----------------------------------------------------------------------------
void EntityButton::SetAsDefaultButton(int state)
{
	if ( _buttonFlags.IsFlagSet( DEFAULT_BUTTON ) != (bool)state )
	{
		_buttonFlags.SetFlag( DEFAULT_BUTTON, state );
...
//-----------------------------------------------------------------------------
void EntityButton::SetArmedSound(const char *sound)
{
	if (sound)
	{
		m_sArmedSoundName = g_EntityButtonSoundNames.AddString(sound);
	}
	else
	{
		m_sArmedSoundName = UTL_INVAL_SYMBOL;
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::SetDepressedSound(const char *sound)
{
	if (sound)
	{
		m_sDepressedSoundName = g_EntityButtonSoundNames.AddString(sound);
	}
	else
	{
		m_sDepressedSoundName = UTL_INVAL_SYMBOL;
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::SetReleasedSound(const char *sound)
{
	if (sound)
	{
		m_sReleasedSoundName = g_EntityButtonSoundNames.AddString(sound);
	}
	else
	{
		m_sReleasedSoundName = UTL_INVAL_SYMBOL;
...
//-----------------------------------------------------------------------------
void EntityButton::SetMouseClickEnabled(MouseCode code,bool state)
{
	if(state)
	{
		//set bit to 1
...
//-----------------------------------------------------------------------------
bool EntityButton::IsMouseClickEnabled(MouseCode code)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetCommand( const char *command )
{
...
//-----------------------------------------------------------------------------
void EntityButton::SetCommand( KeyValues *message )
{
...
// Output : KeyValues
//-----------------------------------------------------------------------------
KeyValues *EntityButton::GetCommand()
{
...
//-----------------------------------------------------------------------------
void EntityButton::FireActionSignal()
{
...
//-----------------------------------------------------------------------------
bool EntityButton::RequestInfo(KeyValues *outputData)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool EntityButton::CanBeDefaultButton(void)
{
    return true;
}
...
//-----------------------------------------------------------------------------
void EntityButton::GetSettings( KeyValues *outResourceData )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::ApplySettings( KeyValues *inResourceData )
{
...
//-----------------------------------------------------------------------------
const char *EntityButton::GetDescription( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnSetState(int state)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnCursorEntered()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnCursorExited()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnMousePressed(MouseCode code)
{
	if (!IsEnabled())
		return;
	...
	{
		surface()->PlaySound(g_EntityButtonSoundNames.String(m_sDepressedSoundName));
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnMouseDoublePressed(MouseCode code)
{
	OnMousePressed(code);
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnMouseReleased(MouseCode code)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnKeyCodePressed(KeyCode code)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void EntityButton::OnKeyCodeReleased( KeyCode keycode )
{
...
//-----------------------------------------------------------------------------
void EntityButton::DrawFocusBorder(int tx0, int ty0, int tx1, int ty1)
{
...
//-----------------------------------------------------------------------------
void EntityButton::SizeToContents()
{
	int wide, tall;
