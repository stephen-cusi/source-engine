...
#include "vscript/matchers.h"
#include "vscript/vscript_singletons.h"
#include "vscript/vscript_vgui.h"
...
bool CScriptMaterialProxy::InitScript()
{
	if (!m_ScriptScope.IsInitialized())
	{
		...
		if (!bResult)
		{
			Msg("VScriptProxy couldn't create ScriptScope!\n" );
			return false;
		}
...
bool VScriptClientInit()
{
	...
	if( scriptmanager != NULL )
	{
		...
		if ( CommandLine()->CheckParm( "-scriptlang", &pszScriptLanguage ) )
		{
			...
			{
				Warning("-scriptlang does not recognize a language named '%s'. virtual machine did NOT start.\n", pszScriptLanguage );
				scriptLanguage = SL_NONE;
			}
		...
		if( scriptLanguage != SL_NONE )
		{
			...
			{
#ifdef MAPBASE_VSCRIPT
				Msg("VSCRIPT CLIENT: Started VScript virtual machine using script language '%s'\n", g_pScriptVM->GetLanguageName() );
#else
		...
		{
			Msg("VSCRIPT CLIENT: Not starting because scripting language is set to 'none'\n" );
		}
#endif
	}
	else
